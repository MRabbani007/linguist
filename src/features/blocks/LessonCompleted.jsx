...
export default function LessonCompleted() {
  ...
  return (
    <div className="flex flex-row items-center justify-center gap-8 h-48">
      <p
        className={
          (isCompleted ? "border-green-500 bg-zinc-50" : "border-zinc-600") +
          " border-4 rounded-full p-4 bg-zinc-400 w-fit duration-500"
        }
      >
        <IoCheckmarkDone
          size={32}
          className={
            (isCompleted ? "text-green-500 scale-125 " : "text-zinc-600") +
            " duration-500"
          }
        />
      </p>
      <p
        className={
          (isCompleted ? "text-green-500 scale-125 visible " : " invisible ") +
          " duration-500 w-fit"
        }
      >
        Lesson Complete!
